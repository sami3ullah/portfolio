...
The fresh-visit intro sits above the page and locks wheel, touch, and keyboard scrolling until its last panel clears. The desktop work row uses CSS sticky positioning with measured scroll space; Lenis and ScrollTrigger share an animation clock, without a second scrub delay. Scroll triggers refresh in document order after effect setup and font loading so the earlier story pin is accounted for before measuring the work section.
